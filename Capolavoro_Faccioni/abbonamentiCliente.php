...
<body>
  ...
  <main>
    <div>
      <center>
        <h1 style="color: white;">I nostri abbonamenti</h1>
      </center>
    </div>

    <div class="wrapper">
      <div class="container">
        <!-- Loop per generare le righe di card degli abbonamenti -->
        <?php
        include "connessione.php";

        $query = "SELECT ID_Abbonamento, Data_Inizio, Data_Fine, Prezzo, Genere, decrizione FROM abbonamento";
        ...
        if ($result->num_rows > 0) {
          ...
          while ($row = $result->fetch_assoc()) {
            ...
            echo '<div class="card">';
            echo '<div class="info">';
            echo '<h2>Genere: ' . $row['Genere'] . '</h2>';
            echo '<p>Descrizione: ' . $row['decrizione'] . '</p>';
            echo '<p>Da: ' . $row['Data_Inizio'] . '</p>';
            echo '<p>a: ' . $row['Data_Fine'] . '</p>';
            echo '<p>Prezzo: ' . $row['Prezzo'] . '€</p>';
            echo '<button type="button" class="btn abbonati" data-id="' . $row['ID_Abbonamento'] . '" data-genere="' . $row['Genere'] . '" data-descrizione="' . $row['decrizione'] . '" data-data-inizio="' . $row['Data_Inizio'] . '" data-data-fine="' . $row['Data_Fine'] . '" data-prezzo="' . $row['Prezzo'] . '">Abbonati</button>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
    ...
    <div id="carrello" style="display: none;">
      ...
      <p id="carrello-totale">Totale: 0€</p>
    </div>
    </div>

    <div id="selected-product-id" style="display: none;"></div>
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    let carrelloTotale = parseFloat(localStorage.getItem('carrelloTotale')) || 0;

    document.querySelectorAll('.btn.abbonati').forEach(button => {
      button.addEventListener('click', function () {
        const id = this.getAttribute('data-id');
        const genere = this.getAttribute('data-genere');
        const descrizione = this.getAttribute('data-descrizione');
        const dataInizio = this.getAttribute('data-data-inizio');
        const dataFine = this.getAttribute('data-data-fine');
        const prezzo = parseFloat(this.getAttribute('data-prezzo'));

        // Controlla se l'articolo è già presente nel carrello
        const isAlreadyInCart = carrelloItems.some(item => item.id === id);

        // Se l'articolo non è già presente, aggiungilo al carrello
        if (!isAlreadyInCart) {
          carrelloItems.push({ id, genere, descrizione, dataInizio, dataFine, prezzo });

          // Salva il carrello nel LocalStorage
          localStorage.setItem('carrelloItems', JSON.stringify(carrelloItems));
          carrelloTotale += prezzo;
          localStorage.setItem('carrelloTotale', carrelloTotale);
